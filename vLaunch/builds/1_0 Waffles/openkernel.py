...
class Launcher(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.notification_image = ImageTk.PhotoImage(file="notifications.png")
        self.notification_button = tk.Button(self.panel_frame, image=self.notification_image, command=self.check_notifications, bg=self.background_color)
        self.notification_button.pack(side=tk.LEFT, padx=20)
    ...
    def show_device_info(self):
        ...
        program_version_label = tk.Label(device_info_frame, text="Версия программы: 1.0 'Waffles' ", font=('Helvetica', 12))
    ...
    def check_notifications(self, event=None):
        self.notification_menu.post(self.notification_button.winfo_rootx(), self.notification_button.winfo_rooty() + self.notification_button.winfo_height())
        self.notification_menu.delete(0, tk.END)
        ...
        self.notification_menu.post(x_position, y_position)
        self.notification_menu.delete(0, tk.END)
        notification_files = ["game.py", "test_not.py", "camera.py", "files.py"]
        ...
        for filename in notification_files:
            if os.path.exists(filename):
                try:
                    ...
                    app_author = getattr(module, 'app_author', None)

                    if app_name is not None and app_version is not None and app_info is not None and app_author is not None:
                        notification_text = f"{app_name} - {app_version}\n{app_info}\nАвтор: {app_author}"
                        self.notification_menu.add_command(label=notification_text, command=lambda text=notification_text: self.show_notification_details(text))

                except Exception as e:
                    print(f"Error processing file {filename}: {e}")

    def show_notification_details(self, notification_text):
        messagebox.showinfo("Детали уведомления", notification_text)

    def create_notification_menu(self):
        self.notification_menu = tk.Menu(self.menubar, tearoff=0, bg="black", fg="white", activebackground="black")
        self.notification_menu.add_command(label="Проверить уведомления", command=self.check_notifications)

        self.menubar.add_cascade(label="Уведомления", menu=self.notification_menu)
